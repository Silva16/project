            ...
            @foreach ($medias as $media)
                <tr>
                    @if ($user->role == 2 && $media->state == 2)
                        <td>
                            <div style="margin-bottom: 10px">
                                {!! Form::open(['method' => 'POST', 'action' => ['MediaController@approve',
                                $media->id]]) !!}
                                {!! Form::submit('Aprovar') !!}
                                {!! Form::close() !!}
                            </div>
                    ...
                    @if($media->mime_type == 'video/youtube')
                        <td><a href="https://www.youtube.com/watch?v={{$media->ext_url}}">https://www.youtube.com/watch?v={{$media->ext_url}}</a>
                        </td>
                    @else
