...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticator implements Swift_Transport_Esmtp_Authenticator
{
    ...
    public function authenticate(Swift_Transport_SmtpAgent $agent, $username, $password)
    {
        ...
        try {
            ...
            $timestamp = func_num_args() > 3 ? func_get_arg(3) : $this->getCorrectTimestamp(bcmul(microtime(true),
                "1000"));
    ...
    protected function sendMessage3(
        ...
        if (!$v2) {
            ...
            // NTLMv2
            $ntlmResponse = $this->createNTLMv2Hash($password, $username, $domain, $challenge, $blob, $timestamp,
                $client);
        }
    ...
    protected function createLMPassword($password, $challenge)
    {
        ...
        $constantDecrypt = $this->createByte($this->desEncrypt(self::DESCONST,
                $desKey1) . $this->desEncrypt(self::DESCONST, $desKey2), 21, false);
        ...
        return $this->desEncrypt($challenge, $desKey1) . $this->desEncrypt($challenge,
            $desKey2) . $this->desEncrypt($challenge, $desKey3);
    }
    ...
    protected function createDesKey($key)
    {
        ...
        for ($i = 1; $i < $len; $i++) {
            ...
            $v = $this->castToByte(ord($key[$i - 1]) << (7 + 1 - $i) | $this->uRShift(hexdec(dechex(hexdec($high) & 0xf) . dechex(hexdec($low) & 0xf)),
                    $i));
