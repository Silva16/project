...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function testLMv1Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $lmv1Result = $this->_invokePrivateMethod('createLMPassword', $login,
            array($password, $this->hex2bin($challenge)));
    ...
    public function testLMv2Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $lmv2Result = $this->_invokePrivateMethod('createLMv2Password', $login,
            array($password, $username, $domain, $this->hex2bin($challenge), $this->hex2bin("ffffff0011223344")));
    ...
    public function testMessage3v1Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $message3 = $this->_invokePrivateMethod('createMessage3', $login,
            array($domain, $username, $workstation, $this->hex2bin($lmResponse), $this->hex2bin($ntlmResponse)));
    ...
    public function testSuccessfulAuthentication()
    {
        ...
        $agent->shouldReceive('executeCommand')
            ->once()
            ->with(base64_encode(
                    $this->_invokePrivateMethod('createMessage3', $ntlm, array(
                            $domain,
                            $username,
                            $this->hex2bin("4d0045004d00420045005200"),
        ...
        $this->assertTrue($ntlm->authenticate($agent, $username . '@' . $domain, $secret,
            $this->hex2bin("30fa7e3c677bc301"), $this->hex2bin("f5ce3d2401c8f6e9")),
    ...
    public function testAuthenticationFailureSendRsetAndReturnFalse()
    {
        ...
        $this->assertFalse($ntlm->authenticate($agent, $username . '@' . $domain, $secret,
            $this->hex2bin("30fa7e3c677bc301"), $this->hex2bin("f5ce3d2401c8f6e9")),
