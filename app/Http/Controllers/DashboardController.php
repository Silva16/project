...
class DashboardController extends Controller {
	...
	public function index()
	{
        ...
        if (($sort = Input::get('sort')) != null && ($order = Input::get('order')) != null && ($filter = Input::get('filter')) != null){
            ...
            $array = $this->getProjects($user, $sort_array[Input::get('sort')], $order_array[Input::get('order')], $filter_array[Input::get('filter')]);
        }
        else {
        ...
        return view('dashboard.list', compact('projects', 'user', 'filter', 'sort', 'order', 'created_by', 'approved_by', 'updated_by'));
    ...
    private function getProjects($user, $sort = 'name', $order = 'asc', $filter = 'all'){
        ...
        return ['projects' => $projects, 'created_by' => $created_by, 'approved_by' => $approved_by, 'updated_by' => $updated_by];
    }
