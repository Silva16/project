...
class CommentsController extends Controller {
	...
	public function index($id)
	{
        ...
        if (($sort = Input::get('sort')) != null && ($order = Input::get('order')) != null && ($filter = Input::get('filter')) != null){
            ...
            $comments = $this->getComment($id, $sort_array[Input::get('sort')], $order_array[Input::get('order')], $filter_array[Input::get('filter')]);
        }
        else {
        ...
        return view('comments.list', compact('comments', 'project', 'user_name', 'approved_by', 'sort', 'order', 'filter', 'user'));
	}
    ...
    private function getComment($id, $sort = 'created_at', $order = 'desc', $filter = 'all'){
        ...
        if ($filter != 'all'){
            $comments = Comment::where('project_id', '=', $id)->where('state', '=', $filter)->orderBy($sort, $order)->get();
